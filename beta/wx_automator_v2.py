...
def find_nearest_neighbors(pole_points, time_points, start_time=None, end_time=None, min_distance_threshold=10, min_time_threshold=1, nn_file='nearest-neighbors.csv'):
    ...
    :param min_time_threshold: Minimum time threshold to consider points as too close.
    ...
    def is_following_direction(prev_point, curr_point, candidate_point):
        # Determine if the candidate point follows the direction from prev_point to curr_point
        prev_to_curr = direction_vector(prev_point, curr_point)
        curr_to_candidate = direction_vector(curr_point, candidate_point)
        return np.dot(prev_to_curr, curr_to_candidate) > 0

    for i, pole in enumerate(pole_points):
        ...
        # Calculate distances between pole point and all time points
        ...
        if not too_close:
            ...
            if len(nearest_neighbors) > 1:
                prev_point = nearest_neighbors[-2][1:3]
                curr_point = nearest_neighbors[-1][1:3]
                candidate_point = [x_pole, y_pole]
                if not is_following_direction(prev_point, curr_point, candidate_point):
                    continue
