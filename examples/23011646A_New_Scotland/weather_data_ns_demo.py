...
def parse_weather_data_times(df):
    ...
    def clean_time_string(time_str):
        """
        Clean the time string by removing unwanted characters.
        
        :param time_str: The original time string.
        :return: A cleaned time string.
        """
        return ''.join(filter(lambda x: x.isdigit() or x in ": AMP", time_str))

    def convert_to_time(time_str):
        """
        Convert the cleaned time string to a datetime.time object.
        
        :param time_str: The cleaned time string.
...
def read_weather_data(weather_data_file, data_columns=[1, 2, 7, 8], skiprows=2):
    ...
    try:
        df = pd.read_excel(weather_data_file, usecols=data_columns, dtype={
            'Time' : 'str',
            'Dir': 'str'
        }, skiprows=skiprows)
    except ValueError:
        df = pd.read_csv(weather_data_file, usecols=data_columns, dtype={
            'Time': 'str',
            'Dir': 'str'
        }, skiprows=skiprows)
        
    # Check if required columns exist and process them
    required_columns = ['Time', 'Out', 'Speed', 'Dir']
    ...
    try:
        df['Speed'] = df['Speed'].astype(float).round(1)
...
def normalize_column_names(df):
    df.columns = [sub(r'\s*\(.*?\)', '', col).strip().lower() for col in df.columns]
    return df
...
def main():
    ...
    nearest = find_nearest_neighbors(pole_points, time_points, start_time, end_time, min_distance_threshold=5, min_time_threshold=2)
    # Write to CSV
